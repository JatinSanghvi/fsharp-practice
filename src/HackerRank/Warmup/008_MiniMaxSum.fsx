...
As you can see, the minimal sum is 1 + 2 + 3 + 4 = 10 and the maximal sum is 2 + 3 + 4 + 5 = 14. Thus, we print these
minimal and maximal sums as two space-separated integers on a new line.
...
[<EntryPoint>]
let main _ =
    let minSum numbers = Array.sum numbers - Array.max numbers
    let maxSum numbers = Array.sum numbers - Array.min numbers

    let numbers = Console.ReadLine().Split [|' '|] |> Array.map int64
    printfn "%d %d" (minSum numbers) (maxSum numbers)
    0